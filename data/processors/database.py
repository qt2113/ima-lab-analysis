...
class DatabaseManager:
    ...
    def query(
        ...
        if start_date:
            _start = pd.to_datetime(start_date, errors='coerce')
            if pd.notna(_start):
                conditions.append("Start >= ?")
                params.append(_start.strftime('%Y-%m-%d'))

        if end_date:
            _end = pd.to_datetime(end_date, errors='coerce')
            if pd.notna(_end):
                conditions.append("Start <= ?")
                params.append(_end.strftime('%Y-%m-%d'))
